<script lang="ts">
	...
	const attendeeId = $page.params.id;
	...
	onMount(async () => {
		try {
			const [attendeesResponse, accommodationsResponse] = await Promise.all([
			...
			const attendee = attendees.find((a) => a.id.toString() === attendeeId);
	...
	async function handleSubmit() {
		...
		try {
			const response = await fetch('/api/admin/attendees', {
				...
				body: JSON.stringify({ id: parseInt(attendeeId), ...form })
			});
...
<div>
	<div class="mb-6 flex items-center justify-between">
		<h1 class="text-3xl font-bold text-irish-navy">Edit Attendee</h1>
		<a
	...
	{#if loadingData}
		<div class="py-8 text-center">
			<p class="text-irish-navy-light">Loading attendee data...</p>
		</div>
	{:else if error}
