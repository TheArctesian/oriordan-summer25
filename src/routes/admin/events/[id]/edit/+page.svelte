	...
	async function handleSubmit() {
		...
		try {
			const response = await fetch('/api/admin/events', {
				...
				body: JSON.stringify({ id: parseInt(eventId), ...form })
			});
...
<div>
	<div class="mb-6 flex items-center justify-between">
		<h1 class="text-3xl font-bold text-irish-navy">Edit Event</h1>
		<a
			href="/admin/events"
	...
	{#if loadingData}
		<div class="py-8 text-center">
			<p class="text-irish-navy-light">Loading event data...</p>
		</div>
	{:else if error}
	...
	{:else}
		<form on:submit|preventDefault={handleSubmit} class="rounded-lg bg-white p-6 shadow">
			<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
				...
				<div>
					<label for="maxAttendees" class="block text-sm font-medium text-gray-700"
						>Max Attendees</label
					>
					<input
						type="text"
